...
double fitness(bool*& x, const int dim_chromo, const vector<std::set<int>>& array_subsets, const vector<double>& w) {
	...
	if (is_disjoint_vector_sets(selected_items))
	{
		return fit;
	}
	...
	{
		double magnitude = (double)clash_magnitude_vector_sets(selected_items);
		double w_aggregate = total_w_all_subsets(w);
		double scaled_factor = fit / w_aggregate;
...
double localsearch(const chromo& curr_best, const int dim_chromo, const vector<std::set<int>>& array_subsets,
		const vector<double>& w, double curr_best_val) {
...
//	{
		for (int j = 0; j < dim_chromo; j++) {
			...
			double curr_fit = fitness(copy_input_best.items, dim_chromo, array_subsets, w);
...
//	{
//		auto file_path = s;
		auto file_path = "uniform100_200_10.txt";
		myfile1.open("cross_mut_found_uniform100_200_10.csv");
		myfile2.open("cross_mut_avg_uniform100_200_10.csv");
...
/*		string_code stringCode = s_mapStringToTestType[(std::string)file_path];
	...
	for (int i = 0; i < pop; i++) { // ch[i] - is ith chromosome; which has properties 'f' - fitness; items - sequence of '0's and '1's
		...
		ch[i].f = fitness(ch[i].items, dimw, array_subsets, w); // chromosome has a member 'f' indicating its fitness
	}
	printf("\n\n");
...
#pragma omp parallel for shared(ch) //mb comment out ?
				...
				for (int i = 0; i < pop; i++)
				{
						...
						{ // elitism - only processes the discarded chromosomes
							...
							if (coin(cross_prob) == 1)
								{ // crossover section
									...
									ch[i].f = fitness(ch[i].items, dimw, array_subsets, w);
									parc += 1;
								}
					...
					if (ch[i].f > best)
					{
						...
						if(b_loc_search) //still set to false - modified the calling part below so this is not called in this version 05.05
						{
							...
							double next_b = localsearch(ch[i], dimw,  array_subsets, w, best);
							...
							while (next_b > best)
							{
								...
								best = next_b;
								next_b = localsearch(ch[i], dimw,  array_subsets, w, best);
							}
						}
					}
				...
				if (best <= 6.2092      + 0.01 && best >= 6.2092    - 0.01)
				{
					myfile1 << best;
					myfile1 << "\n";
					myfile1 << cross_prob;
				...
				if (best_arr_size < 50000) continue; //continue to next generation; otherwise, check the no_changes clause below

				if (best_arr_size % 5000 == 0)
				{
					mut_prob += 0.1; // may COMMENT this OUT
					if (best_arr[44999] == best_arr[49999])
					{
						no_changes = true;
